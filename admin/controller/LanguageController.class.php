...
class LanguageController extends NesoteController
{
	...
	function storeinfoAction()
	{
		...
		$char_encoding=trim((string) $_POST['char_encoding']);
		$language_code=trim((string) $_POST['lang_code']);
		...
		$tot=$db->total("nesote_email_languages","language=? or language_script=? ",[$language,$script]);
		if($tot!=0)
		{
		...
		function editAction()
		{
			...
			if (($_SERVER['HTTP_HOST'] == "www.libewebportal.com" || $_SERVER['HTTP_HOST'] == "libewebportal.com" || $_SERVER['HTTP_HOST'] == "www.libewebmaildemo.com" || $_SERVER['HTTP_HOST'] == "libewebmaildemo.com") && $lang_id < 4) {
		...
		function storeeditinfoAction()
		{
			...
			$lang_script=trim((string) $_POST['script']);
			$char_encod=trim((string) $_POST['char_encoding']);
			$lang_code=trim((string) $_POST['lang_code']);
			...
			$db= new NesoteDALController();
			$tot=$db->total("nesote_email_languages","(language=? or language_script=? or lang_code=?) and id!=?",[$lang,$lang_script,$lang_code,$lang_id]);
			if($tot!=0)
			{
				header("Location:".$this->url("message/error/1042"));
				exit(0);
		...
		function deleteAction()
		{
			...
			if (($server == "www.libewebportal.com" || $server == "libewebportal.com") && $lang_id < 4) {
                header("Location:".$this->url("message/error/1023"));
                exit(0);
            }
			$db= new NesoteDALController();
		...
		function messagesAction()
		{
			...
			$lang_id = $_POST['lang'] ?? $this->getParam(1);
			...
			$this->setValue("lang_id",$lang_id);
			$db= new NesoteDALController();
			$tot=$db->total("nesote_email_languages","id=?",[$lang_id]);
			if($tot==0)
			{
		...
		function getmessage($mid,$lid)
		{
			...
			$db->select("nesote_email_messages");
			$db->fields("msg_id,wordscript");
			$db->where("lang_id=? and msg_id=?",[$lid,$mid]);
			$result=$db->query();
		...
		{
			$db= new NesoteDALController();
			$db->select("nesote_libearticle_category_message");
			$db->fields("cat_id,message");
			$db->where("lang_id=? and cat_id=?",[$lid,$mid]);
			$result=$db->query();
			$row=$db->fetchRow($result);
			return $row[1];
		}
		...
		function getmonthsmessage($mid,$lid)
		{
			...
			$db->where("lang_id=? and month_id=?",[$lid,$mid]);
			$result=$db->query();
			$row=$db->fetchRow($result);
			return $row[1];
		}
		...
		{
			$db= new NesoteDALController();
			$db->select("nesote_libearticle_parameters_messages");
			$db->fields("cust_id,message");
			$db->where("lang_id=? and cust_id=?",[$lid,$mid]);
			$result=$db->query();
			$row=$db->fetchRow($result);
			return $row[1];
		}
		...
		function storemessageinfoAction()
		{
			...
			while($row=$result->fetchRow())
			{
				if($_POST['script'.$row[0]]!="")
				{
					...
					$tot=$db->total("nesote_email_messages","lang_id=? and msg_id=?",[$lang_id,$row[0]]);
					//$total=$db->total("nesote_email_messages","lang_id=? and msg_id<>? and wordscript=?",array($lang_id,$row[0],$wordscript));
					//$total=$db->total("nesote_email_messages","lang_id=? and msg_id<>?",array($lang_id,$row[0]));
					//if($total==0)
					//{
						if($tot==0)
						{
							...
							if($quotes==1)
							{
								$messages=stripslashes((string) $wordscript);
								//echo $messages;
						...
						else
						{
							$db->select("nesote_email_messages");
							$db->fields("id");
							$db->where("msg_id=? and lang_id=?",[$row[0],$lang_id]);
							$result1=$db->query();
		...
		function store_messagesAction()
		{
			...
			$tot=$db->total("nesote_email_messages","lang_id=? and wordscript=?",[$id,$message]);
			if($tot!=0)
			{
		...
		function messages_codesAction()
		{
			...
			$currentpage=1;
			//$db=new NesoteDALController();
			//$tot=$db->total(array("a"=>"libe_ipns","b"=>"libe_users"),"a.userid=b.userid  and $condition", $conditionarray);
			$db=new NesoteDALController();
			$tot=$db->total(["a"=>"nesote_email_languages","b"=>"nesote_email_messages"],"a.language=? and  a.id=b.lang_id","English");
			...
			$db= new NesoteDALController();
			$db->select(["a"=>"nesote_email_languages","b"=>"nesote_email_messages"]);
		...
		function parametersAction()
		{
			...
			$db->select("nesote_libearticle_custom_fields");
		...
		function storeparameterinfoAction()
		{
			...
			while($row=$result->fetchRow())
			{
				if($_POST['script'.$row[0]]!="")
				{
					...
					$wordscript=$_POST['script'.$row[0]];
					$tot=$db->total("nesote_libearticle_parameters_messages","lang_id=? and cust_id=?",[$lang_id,$row[0]]);
					//echo $db->getQuery();
					//$total=$db->total("nesote_libearticle_parameters_messages","lang_id=? and cust_id<>? and message=?",array($lang_id,$row[0],$wordscript));
					//echo $db->getQuery();
					...
					if($tot==0)
					{
						...
						$mdl=$this->modelInstance("nesote_libearticle_parameters_messages");
					...
					else
					{
						$db->select("nesote_libearticle_parameters_messages");
						$db->fields("id");
						$db->where("cust_id=? and lang_id=?",[$row[0],$lang_id]);
						$result1=$db->query();
						$row1=$result1->fetchRow();
						$mdl=$this->modelInstance("nesote_libearticle_parameters_messages");
						$mdl->load($row1[0]);
				...
				if($_POST['script'.$row[0]]=="")
				{
					...
					if($tot!=0)
					{
						...
						$result1=$db->query();
						$row1=$result1->fetchRow();
						$db->delete("nesote_libearticle_parameters_messages");
						$db->where("id=?",$row1[0]);
						$db->query();
		...
		function getparameterfiledvalues($value)
		{
			...
			for($i=0;$i<$counter;$i++)
			{
				$value = $i == 0 ? $value.$elements[$i] : $value.",".$elements[$i];
		...
		function categoriesAction()
		{
			...
			$db->select("nesote_libearticle_categories");
		...
		function storecategoryinfoAction()
		{
			...
			while($row=$result->fetchRow())
			{
				if($_POST['script'.$row[0]]!="")
				{
					...
					$wordscript=$_POST['script'.$row[0]];
					$tot=$db->total("nesote_libearticle_category_message","lang_id=? and cat_id=?",[$lang_id,$row[0]]);
					$total=$db->total("nesote_libearticle_category_message","lang_id=? and cat_id<>? and message=?",[$lang_id,$row[0],$wordscript]);
					//echo $db->getQuery();
					if($total==0)
					{
						if($tot==0)
						{
							$mdl=$this->modelInstance("nesote_libearticle_category_message");
							$mdl->setCat_id($row[0]);
							$mdl->setLang_id($lang_id);
							if($quotes==1)
							{
						...
						else
						{
							$db->select("nesote_libearticle_category_message");
							$db->fields("id");
							$db->where("cat_id=? and lang_id=?",[$row[0],$lang_id]);
							$result1=$db->query();
				...
				if($_POST['script'.$row[0]]=="")
				{
					...
					if($tot!=0)
					{
						...
						$result1=$db->query();
						$row1=$db->fetchRow($result1);
						$db->delete("nesote_libearticle_category_message");
						$db->where("id=?",$row1[0]);
						$db->query();
		...
		function monthsAction()
		{
			...
			$lang_id = $_POST['lang'] ?? $this->getParam(1);
			$this->setValue("lang_id",$lang_id);
			//			if(($server=="www.libewebmaildemo.com") || ($server=="libewebmaildemo.com"))
			//			{
			//				if($lang_id<4)
				//				{
				//					header("Location:".$this->url("message/error/1023"));
			...
			function storemonthinfoAction()
			{
				...
				if (($_SERVER['HTTP_HOST'] == "www.libewebportal.com" || $_SERVER['HTTP_HOST'] == "libewebportal.com") && $lang_id < 4) {
                    header("Location:".$this->url("message/error/1023"));
                    exit(0);
                }
				$lang_id=$this->getParam(1);
				...
				while($row=$result->fetchRow())
				{
					if($_POST['script'.$row[0]]!="")
					{
						...
						$wordscript=$_POST['script'.$row[0]];
						$tot=$db->total("nesote_email_months_messages","lang_id=? and month_id=?",[$lang_id,$row[0]]);
						$total=$db->total("nesote_email_months_messages","lang_id=? and month_id<>? and message=?",[$lang_id,$row[0],$wordscript]);
						//echo $db->getQuery();
						if($total==0)
						{
							if($tot==0)
							...
							else
							{
								$db->select("nesote_email_months_messages");
								$db->fields("id");
								$db->where("month_id=? and lang_id=?",[$row[0],$lang_id]);
								$result1=$db->query();
								$row1=$db->fetchRow($result1);
								$mdl=$this->modelInstance("nesote_email_months_messages");
								$mdl->load($row1[0]);
					...
					if($_POST['script'.$row[0]]=="")
					{
						...
						if($tot!=0)
						{
							...
							$result1=$db->query();
							$row1=$db->fetchRow($result1);
							$db->delete("nesote_libearticle_months_messages");
							$db->where("id=?",$row1[0]);
							$db->query();
			...
			{
				$count=	substr_count((string) $wordscripts,"{");
				if($count==0)
				{
