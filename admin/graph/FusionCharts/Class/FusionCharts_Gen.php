...
class FusionCharts{
	...
	public $SWFPath;                  # FusionCharts height
	...
	public $chartParams = [];     #List of Chart Parameters
	public $chartParamsCounter;		#Number of Chart parameters
	
	public $categoriesParam;           # Categories Parameter Setting 
	public $categoryNames = [];   # Category array for storing Category set
	public $categoryNamesCounter;      # Category array counter
	
	public $dataset = [];         # dataset array
	public $datasetParam = [];    # dataset parameter setting array
	public $datasetCounter;            # dataset array counter
	public $setCounter;                # set array counter 


    # trendLines array
    public $trendLines = [];      # trendLines array
	public $tLineCounter;              # trendLines array counter

    #chart messages
	public $chartMSG;

	public $chartSWF = [];		# Charts SWF array
	public $arr_FCColors = []; 	# Color Set to be applied to dataplots
...
#############============ For Gantt Chart  ============================
    # Gantt categories
	public $GT_categories = [];
	public $GT_categories_Counter;
	public $GT_categoriesParam = [];
...
#############============ For Future Use (start)============================
	...
	public $MSSDataset = [];       # dataset array for MSStackedColumn2D
	public $MSSDatasetParams = [];  # MSSDataset parameter setting
	...
	# lineset 
	public $lineSet = [];         # lineSet array
	public $lineSetParam = [];    # lineSet Parameter setting array
	...
	# vtrendLines array
    public $vtrendLines = [];     # vtrendLines array
	public $vtLineCounter;             # vtrendLines array counter

    # style array
    public $styles = [];           # styles array
...
#############============ For Future Use (end)============================
	 ...
	 function __construct($chartType="column2d",public $width="400",public $height="300",$chartID="",public $wMode=""){
		  ...
		  # Setting Chart name
		  $this->chartType=strtolower((string) $chartType);
...
##------------ PUBLIC FUNCTIONS ----------------------------------------------------------------
	 ...
	 function setDataBaseType($dbType){
	    $this->DataBaseType=strtolower((string) $dbType);
	 }
	 ...
	 function setChartParams($strParam){
	   $listArray=explode($this->del,(string) $strParam);
	 ...
	 function addChartData($value="",$setParam="",$vlineParam = "" ){
		 ...
		 if($this->seriesType >= 1 && $this->seriesType <= 2){
		 ...
		 if ($this->seriesType==1){
		      ...
		      $this->setCounter++;
		 }elseif($this->seriesType > 1 && $this->seriesType < 4){
	 ...
	 function addDatasetsFromDatabase($query_result, $ctrlField, $valueField,$datsetParamArray="",$link=""){
			 ...
			 if($this->DataBaseType=="mysql"){ 	
				 ...
				 $FieldArray=explode($this->del,(string) $valueField);
				 if(count($FieldArray)>1){
					 ...
					 foreach ($FieldArray as $FieldName) {
						   ...
						   $tempParam = $k < $arrLimit ? $datsetParamArray[$k] : "";
						   ...
						   while($row = mysql_fetch_array($query_result)){ 
						        ...
						        if($link==""){
							      ...
							      $paramset="link=" . urlencode((string) $this->getLinkFromPattern($row,$link));
								}
					 ...
					 while($row = mysql_fetch_array($query_result)){
						   ...
						   if ($tempContrl!=$row[$ctrlField]) {
                               $tempParam = $i <= $arrLimit ? $datsetParamArray[$i-1] : "";
                               # Add Dataset with adddataset() function
		   ...
		   while($row = mysql_fetch_array($query_result)){
			          ...
			          if($LinkPlaceHolder==""){
						  $paramset="";
						   ...
						   # Getting link
						  $paramset="link=" . urlencode((string) $this->getLinkFromPattern($row,$LinkPlaceHolder));
					   }
					   if (($strParam="") !== ''){
						 $strParam=$paramset;
					   }else{
	...
	function addTrendLine($strParam){
	 ...
	 function getXML(){
		...
		if($this->seriesType==1){
				   if (gettype(strpos((string) $this->chartType,"line")) !== "boolean" && !str_contains((string) $strChartParam,"lineColor")) {
                       $colorSet=$this->getColor(0);
                       $this->setChartParams("lineColor=" . $colorSet );
                   }
				   if (gettype(strpos((string) $this->chartType,"area")) !== "boolean" && !str_contains((string) $strChartParam,"areaBgColor")) {
                       $colorSet=$this->getColor(0);
		...
		if ($this->seriesType >= 0 && $this->seriesType <= 4) {
	...
	function renderChartJS($chartSWF, $strURL, $strXML, $chartId, $chartWidth, $chartHeight, $debugMode=true, $registerWithJS=false, $setTransparent="") {
		...
		return $strHTML . "<!-- END Script Block for Chart $chartId -->\n";
...
##------------ PRIVATE FUNCTIONS ----------------------------------------------------------------
	 ...
	 function getDatasetXML(){
	   ...
	   return match ($this->seriesType) {
           1 => $this->getSSDatasetXML(),
           2, 3 => $this->getMSDatasetXML(),
           4 => $this->getMSStackedDatasetXML(),
           default => null,
       };
	 }
	 ...
	 function getCategoriesXML(){
	   if($this->seriesType>1){
		   ...
		   foreach($this->categoryNames as $part_name){
				   # adding elements 
                   if($part_name != "" && $part_name != "Array"){ 
					 $partXML .= $part_name;
				   } 
		   }
		   }
	 ...
	 function getSSDatasetXML(){
	   if($this->seriesType==1){
		   $partXML="";
		   foreach($this->dataset as $part_name){
			   # adding elements 
               if($part_name != "" && $part_name != "Array"){ 
				 $partXML .= $part_name;
			   } 
	 ...
	 function getMSDatasetXML(){
	   if($this->seriesType>1){
		   ...
		   foreach($this->dataset as $part_type => $part_name){
			   ...
			   foreach($this->dataset[$part_type] as $part_name1){
					   # Adding elements 
                       if($part_name1 != "" && $part_name1 != "Array"){ 
						 $partXML .= $part_name1;
					   } 
			   }
			   $partXML .="</dataset>";
	 ...
	 function setSSMSDataArray($value="",$setParam="",$vlineParam = "" ){
		 ...
		 if($vlineParam==""){
		   ...
		   if ($this->UserColorON == true) {
               if($this->seriesType == 1 && (gettype(strpos((string) $this->chartType,"line")) === "boolean" && gettype(strpos((string) $this->chartType,"area")) === "boolean") && !str_contains(strtolower((string) $strParam)," color")){
				  $colorSet=" color='" . $this->getColor($this->setCounter) . "' ";   
			   }
           } elseif ($this->seriesType==1 && (gettype(strpos((string) $this->chartType,"pie")) === "boolean" && gettype(strpos((string) $this->chartType,"line")) === "boolean" && gettype(strpos((string) $this->chartType,"area")) === "boolean")) {
               if(!str_contains(strtolower((string) $strParam)," color")){
					 $colorSet=" color='" . $this->getColor($this->setCounter) . "' ";
				  }
           }
  ...
  # ----- ----------    -----  Misc utility functions  ---- ------ -----------
	 ...
	 function encodeSpecialChars($strValue){
		...
		$strValue=preg_replace($pattern, "%25", (string) $strValue);
	    ...
	    if ($this->encodeChars==true){
			...
			$strValue=preg_replace("/\<\/a/i", "%26lt;/A", (string) $strValue);
			
			$strValue=preg_replace("/\<i/i", "%26lt;I", (string) $strValue);
			$strValue=preg_replace("/\<\/i/i", "%26lt;/I", (string) $strValue);
			
			$strValue=preg_replace("/\<u/i", "%26lt;U", (string) $strValue);
			$strValue=preg_replace("/\<\/u/i", "%26lt;/U", (string) $strValue);
			
			$strValue=preg_replace("/\<li/i", "%26lt;LI", (string) $strValue);
			$strValue=preg_replace("/\<\/li/i", "%26lt;/LI", (string) $strValue);
			
			$strValue=preg_replace("/\<font/i", "%26lt;FONT", (string) $strValue);
			$strValue=preg_replace("/\<\/font/i", "%26lt;/FONT", (string) $strValue);
			
			$strValue=preg_replace("/\<p/i", "%26lt;P", (string) $strValue);
			$strValue=preg_replace("/\<\/p/i", "%26lt;/P", (string) $strValue);
			
			$strValue=preg_replace("/\<br/i", "%26lt;BR", (string) $strValue);
			
			$strValue=preg_replace("/\<b/i", "%26lt;B", (string) $strValue);
			$strValue=preg_replace("/\<\/b/i", "%26lt;/B", (string) $strValue);
			...
			$strValue=preg_replace("/\<\/a/i", "&lt;/A", (string) $strValue);
			
			$strValue=preg_replace("/\<i/i", "&lt;I", (string) $strValue);
			$strValue=preg_replace("/\<\/i/i", "&lt;/I", (string) $strValue);
			
			$strValue=preg_replace("/\<u/i", "&lt;U", (string) $strValue);
			$strValue=preg_replace("/\<\/u/i", "&lt;/U", (string) $strValue);
			
			$strValue=preg_replace("/\<li/i", "&lt;LI", (string) $strValue);
			$strValue=preg_replace("/\<\/li/i", "&lt;/LI", (string) $strValue);
			
			$strValue=preg_replace("/\<font/i", "&lt;FONT", (string) $strValue);
			$strValue=preg_replace("/\<\/font/i", "&lt;/FONT", (string) $strValue);
			
			$strValue=preg_replace("/\<p/i", "&lt;P", (string) $strValue);
			$strValue=preg_replace("/\<\/p/i", "&lt;/P", (string) $strValue);
								
			$strValue=preg_replace("/\<br/i", "&lt;BR", (string) $strValue);
					
			$strValue=preg_replace("/\<b/i", "&lt;B", (string) $strValue);
			$strValue=preg_replace("/\<\/b/i", "&lt;/B", (string) $strValue);
	    ...
	    return str_replace("₣","%e2%82%a3",$strValue);
    ...
    function getLinkFromPattern($row,$tempLink){			
	 ...
	 function validateParam($paramValue){
	     if (count($paramValue)>=2) {
             return trim((string) $paramValue[0]) !== "";
         } else{
		    return false;
		 }
	 }
	 ...
	 function getSeriesType(){
	    $sValue = is_array($this->chartSWF[$this->chartType]) ? $this->chartSWF[$this->chartType][1] : 1;
	 ...
	 function addColors($ColorList)
        {
		   $listArray=explode($this->del, (string) $ColorList);
	...
	#-- GANTT XML genetators -------------------------------------------
	 ...
	 function getGanttCategoriesXML(){
		   ...
		   foreach($this->GT_categories as $part_type => $part_name){
			   ...
			   foreach($this->GT_categories[$part_type] as $part_name1){
					   if($part_name1!=""){ 
						 # adding elements 
							 $partXML .= $part_name1;
					   } 
	 ...
	 function getProcessesXML(){
		   ...
		   $partXML="<processes " . $this->GT_processes_Param . " >";
		    foreach($this->GT_processes as $part_name){
				   if($part_name!=""){ 
					 # adding elements 
	 ...
	 function getTasksXML(){
		   ...
		   $partXML="<tasks " . $this->GT_Tasks_Param . " >";
		    foreach($this->GT_Tasks as $part_name){
				   if($part_name!=""){ 
					 # adding elements 
	 ...
	 function getConnectorsXML(){
		   ...
		   $partXML="<connectors " . $this->GT_Connectors_Param . " >";
		    foreach($this->GT_Connectors as $part_name){
				   if($part_name!=""){ 
					 # adding elements 
	 ...
	 function getMilestonesXML(){
		   ...
		   foreach($this->GT_Milestones as $part_name){
				   if($part_name!=""){ 
					 # adding elements 
					 $partXML .= $part_name;
					 $c++;
	 ...
	 function getLegendXML(){
		   ...
		   foreach($this->GT_legend as $part_name){
				   if($part_name!=""){ 
					 # adding elements 
					 $partXML .= $part_name;
					 $c++;
	 ...
	 function getGanttDatatableXML(){
		   ...
		   foreach($this->GT_datatable as $part_type => $part_name){
			   ...
			   foreach($this->GT_datatable[$part_type] as $part_name1){
					   if($part_name1!=""){ 
						 # adding elements 
							 $partXML .= $part_name1;
					   } 
		   ...
		   return "<dataTable " . $this->GT_datatableParam . " >" . $partXML . "</dataTable>";
	 }
  ...
  ##---------PRIVATE functions ----------------------------------------------------
     ...
     */
	 function getStylesXML(){
		...
		return $pos !== false ? "<styles>" . $lineXML . "</styles>" : "";
	 }
