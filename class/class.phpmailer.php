 ...
 */
  ...
  protected function MailSend($header, $body) {
    ...
    } elseif ($this->SingleTo === true && count($toArr) > 1) {
        foreach ($toArr as $val) {
          $rt = @mail($val, $this->EncodeHeader($this->SecureHeader($this->Subject)), $body, $header, $params);
        }
    } else {
  ...
  */
  ...
  public function CreateHeader() {
    ...
    // Add custom headers
    $counter = count($this->CustomHeader);

    // Add custom headers
    for($index = 0; $index < $counter; $index++) {
      $result .= $this->HeaderLine(trim((string) $this->CustomHeader[$index][0]), $this->EncodeHeader(trim((string) $this->CustomHeader[$index][1])));
    }
  ...
  */
  ...
  public function EncodeQ ($str, $position = 'text') {
    ...
    switch (strtolower($position)) {
      case 'phrase':
        $encoded = preg_replace_callback("/([^A-Za-z0-9!*+\\/ -])/", fn($matches) => '=' . sprintf('%02X', ord($matches[1])), (string) $encoded);
        break;
      case 'comment':
        $encoded = preg_replace_callback("/([\\(\\)\"])/", fn($matches) => '=' . sprintf('%02X', ord($matches[1])), (string) $encoded);
      case 'text':
      default:
        // Replace every high ascii, control =, ? and _ characters
        //TODO using /e (equivalent to eval()) is probably not a good idea
        $encoded = preg_replace_callback('/([\000-\011\013\014\016-\037\075\077\137\177-\377])/',
              fn($matches) => '=' . sprintf('%02X', ord($matches[1])), (string) $encoded);
        break;
    }
  ...
  public function MsgHTML($message, $basedir = '') {
    ...
    $this->IsHTML(true);
    $this->Body = $message;
    $textMsg = trim(strip_tags((string) preg_replace('/<(head|title|style|script)[^>]*>.*?<\/\\1>/s','',(string) $message)));
    if ($textMsg !== '' && $textMsg !== '0' && empty($this->AltBody)) {
