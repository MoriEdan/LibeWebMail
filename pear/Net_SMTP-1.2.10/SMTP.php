...
class Net_SMTP
{
    ...
    function _send($data)
    {
        ...
        if ((new PEAR())->isError($error = $this->_socket->write($data))) {
            return (new PEAR())->raiseError('Failed to write to socket: ' .
                                    $error->getMessage());
        }
    ...
    function mailFrom($sender, $params = null)
    {
        ...
        if ((new PEAR())->isError($error = $this->_put('MAIL', $args))) {
            return $error;
        }
        if ((new PEAR())->isError($error = $this->_parseResponse(250))) {
            return $error;
    ...
    function rcptTo($recipient, $params = null)
    {
        ...
        if ((new PEAR())->isError($error = $this->_parseResponse([250, 251]))) {
            return $error;
        }
    ...
    function data($data)
    {
         ...
         * message size". */
        if (isset($this->_esmtp['SIZE']) && $this->_esmtp['SIZE'] > 0 && strlen($data) >= $this->_esmtp['SIZE']) {
            $this->disconnect();
            return (new PEAR())->raiseError('Message size excedes the server limit');
        }
