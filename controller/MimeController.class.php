...
$md5_references=md5($mail_references);
				...
				preg_match_all('/<folderid>(.+?)<\/folderid>/i',(string) $mail_references,$folderArray_last);
...
$md5_references=md5($mail_references);
  ...
  $returnString.=$inbox."/".$spam."/".$trash."/".$cus_string;
	...
	function downloadattachmentAction()
	{
		...
		$path="attachments/$folderid/$tablenumber/$mailid/$filename";//"userdata/attachments";
...
function getlang_id($lang_code)
	...
	function getDecodedFrom($from)
	{
		...
		if(count($charset)!=0)
		{
			preg_match_all('/=\?(.+?)\?=/i',(string) $from,$charsets1);//return count($charsets1[1]);
			$fr="";$fr1="";
